...
<body>
    <?php require('../helper/SignupFormValidation.php');?>
    
    <h1 class="text-center mt-5 mb-3 p-3" >Create Account</h1>
     <div class="container border border-light rounded shadow" style="max-width:60vh;min-height:30vw">
        <form name="signup-form" class="mt-4"  method="post" action = "<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>" method="post" novalidate >
              ...
              <div>
                  ...
                  <input type="password" maxlength="50" name="password" class="form-control p-2" id="Password1" placeholder="Password"  title="Password must be 8-50 chatacter long and contain at least one uppercase letter, one lowercase letter, one digit, and one special character" >
                  <span class="text-danger"><?php echo isset($passwordErr) ? $passwordErr : null ?></span>
              </div>
              <div class="d-flex flex-row justify-content-between mt-5">
                  <button type="submit" class="btn btn-lg btn-success p-2">Sign Up</button>
                  <a  class="h5 p-2" href="http://localhost/authentication/views/signin.php">Sign In</a>
              </div>
          </form>
     </div>
     <style>
